    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag (v2f i) : COLOR {
				...
				float illum = tex2D(_Illumin, i.tex.xy).a;
				...
				color2 += spec*color2;
				color1.rgb*=albedo;
				color1 *= _ColorIntensity;
				color2 *= _ColorIntensity;
				float4 col;
				//if(cutout>_Cutout) col = color1;
				//else col = color2;
				col = color1 * cutout + color2 *(1-cutout);
			    return col * _LightColor0 + col*illum;
            }
         ...
         CGPROGRAM
            ...
            float4 frag (v2f i) : COLOR {
				...
				* max(0.0, dot(normalDirection, lightDirection));
				...
				//if(cutout>_Cutout) col = color1;
				//else col = color2;
				col = (color1 * cutout + color2*(1-cutout))*(attenuation-0.2)*3;
				return col + col*illum;
            }
